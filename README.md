...
| 最大持仓数 | 6 |
| 常规行业最大权重 | 75% |
| 已确认恢复组合的行业/相关集中上限 | 92% |
| 最小权重变化 | 5% |
| 最小交易金额 | 20,000 元 |
| 最大成交量参与率 | 0.5% |

92% 恢复集中额度只适用于原生成交与当前事件仍证明的 recovery owner 恢复权，不扩大普通
恢复权限，也不要求投满；逐股原权利、单票、风险总仓、待成交责任和现金仍共同限制。
资格、已确认持仓修复与实际成交分别核验，详见[冲击修复](docs/STRATEGY.md#冲击修复)。
