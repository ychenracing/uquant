...
数量重新经过风险和组合分配，不会把王冠转给第二名。候选或路线失效、数据身份变化、永久退出
...
任意时刻最多一个 `ACTIVE` epoch。旧 owner 完整退出并终结其 epoch 后，系统才能在现有
cooldown、资格、Risk 和执行边界内创建后继 grant。同一 owner 重新获得所有权也必须经过
