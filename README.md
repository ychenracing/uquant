...
带 `ACCOUNT_INPUT.json` 的历史数据目录使用原始成交价格和逐日因果信号，登记日权益、
分红、股份交付和有明确依据的扣税由原生账户处理。输入字段、真实公告小案例和拒绝条件见
[股份与现金输入](docs/ACCOUNT_INPUT.md)。冻结前复权结果仍保留其原始价格语义，不能作为
真实股份账户的验收证明；默认生产成员与策略参数不因选择数据目录而扩展。
