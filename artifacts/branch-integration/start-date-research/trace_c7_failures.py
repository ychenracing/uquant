...
def run(case):
    ...
    out = Path("../diagnostic-c7-failures")
...
with concurrent.futures.ProcessPoolExecutor(max_workers=3) as p:
    list(p.map(run, [("offset5", 1, False), ("offset5-cost2", 2, False), ("loo308", 1, True)]))
