...
class CoreTransfer:
    ...
    def from_key(cls, key: str) -> CoreTransfer:
        ...
        if (prefix != "core_transfer" or not separator or not arrow or not incumbent
                or not challenger or ":" in pair or "->" in challenger):
    ...
    def observe(self, account: AccountState, *, session: int, previous_session: int,
                qualified: bool) -> None:
...
def transfers(account: AccountState) -> tuple[CoreTransfer, ...]:
    return tuple(CoreTransfer.from_key(key) for key in account.replacement_tenure
