...
for case in ('full', '2024', 'loo308', 'no_three'):
    ...
    with gzip.open(candidate_root / f'{case}.json.gz', 'rt') as stream:
    ...
    for row in events['rows']:
        ...
        for name, data in (('baseline', replay), ('candidate', candidate)):
            row[name + '_same_signal_buy_fills'] = [
                {k:f[k] for k in ('fill_date', 'shares', 'price', 'commission',
                                  'transfer_fee', 'slippage_cost', 'mechanism')}
                for f in data['result']['final_account']['fills']
                if f['signal_date'] == row['date'] and f['symbol'] == row['symbol'] and f['side'] == 'BUY']
        row['paired_complete_portfolio_delta'] = candidate['result']['final_equity'] - curve[-1]['equity']
...
(HERE / 'EVENT_SUMMARY.json').write_text(json.dumps(dict(cases=summaries,
    evidence_scope='All native holding events at the existing add stage, including blocked and accepted requests. '
                   ...
                   'Buy quantities are actual fills from independently evolving paths, not injected counterfactuals. '
                   'Complete-window portfolio delta is not additive per-event profit. '
