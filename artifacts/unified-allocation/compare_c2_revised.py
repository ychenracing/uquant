"""Apply the user's post-observation ceil amendment; original comparison stays frozen."""
import argparse
import gzip
...
a = p.parse_args()
contract = json.loads(Path(__file__).with_name('ACCEPTANCE_C2_REVISED.json').read_text())
rows = []
...
report = dict(unrun_cases=sorted(set(contract['cases'])-{r['case'] for r in rows}), active_route='A', contract='ACCEPTANCE_C2_REVISED.json', rows=rows, paired_cases=len(rows), complete=complete, geometric_wealth_ratio=geometric if len(regular) == 6 else None,
