...
本次用户明确授权采用 `benchmarks/generalization_tradeoff_acceptance.json`：允许有限历史收益让渡，换取可量化的跨 AI 主线与尾部稳健性改善。固定历史原版为 `cf8fecff76564fd4ed87faa0da336a06d433fd93`，不得用较弱研究对照替代。

- 五股票池、九原有窗口采用总体收益保留预算，并披露每个单元的明显退化；主账户仍要求财富至少15倍、回撤不超过31%、订单不超过44。
- 历史与候选的公共生产回测采用同股票输入、窗口和数据；34次公共回测移除用于配对比较。现代原生全角色移除单独验证，不混算历史改善。
- 必须证明下四分位收益改善，或在有限收益代价下改善尾部回撤；不能只靠缩仓使所有收益降低。
- 被动持有财富比例及周期/主导者数量作诊断，不再作为这份经济验收的独立一票否决。因果执行、资金和持仓守恒、成本、真实订单、输入身份与证据完整性仍必须验证。
- 原有 Performance、Absolute、Ownership 的历史合同和失败报告保留，旧报告不改标通过。新验收结果必须单独绑定精确候选与原始证据；缺失或无效证据不得通过。实际仓库保护仍需满足。

具体数值、总体、分位算法与压力场景定义见该合同。当前实施与证据状态见原 PR #67；文档或合同提交本身不代表经济通过。

以下保留原生 Absolute 证据链的执行与核验说明。其事实、身份与会计检查继续适用；旧经济政策判断与当前取舍判断分别报告。
