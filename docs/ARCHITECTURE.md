...
当日已完成自身长期证明的 `persistent_industry + FULL_COHORT` 初始队列，使用原生完整形成预算；
各成员仍须具备同日同身份的证明、原确认次数和流动性。该完整长期证明可替代普通入场结构，
但普通路线及其他形成仍受原结构检查约束。完整实际成交的该队列成员仍接受已有的确认结构退出，
当前周期主导者利润锁定例外保持不变；战略 ATR、灾难和账户全局风险退出仍有效。
...
普通持有结构确认由 leader lifecycle 统一消费当期科技基准 60 日收益，与个股同期收益区分共同市场损伤和个体退化；当前买入资格不承担持有授权。恢复队列在唯一资本账本内按当日百分比 ATR 调整受限新请求份额，原请求上限和实际执行责任保持独立约束。
