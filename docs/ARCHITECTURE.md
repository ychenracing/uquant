...
`capital_peak` 保存真实终身高水位；`operating_peak` 保留短周期恢复语义。风险层唯一的 `deployed_drawdown` 所有者维护 `deployed_peak`，仅在实际持仓结束时重新定基。该字段随账户保存、严格校验并在重启时读回，不参与资金记账。旧 schema 8 账户缺少该派生字段时，仍有正持仓则保守采用已记录的资本高水位，空仓则采用真实现金；不改写既有经济字段、成交或历史曲线。
...
确认恢复层维护原有持仓、待成交和风险恢复权，并返回新增请求；pipeline 将恢复请求与普通
请求放入同一资金循环，恢复优先、类内按领涨分数和代码排序。确认恢复活跃本身不再排斥
普通新请求；战术生命周期、CAUTION 探针、风险冻结和未结清责任仍保留各自准入限制。
该分配顺序不改变下一开盘的实际订单执行顺序，跳空和部分成交仍可能产生现金竞争。
...
生产CLI通过唯一严格JSON读取函数接入公开配置；构造校验检查公开范围和固定规则自身不变量，
实际预算、持仓名额和订单消费负责限制部署。降低容量不缩短战略或恢复成员的确认。
Markdown和离线HTML只读取同一次决策、原生条件记录与账户事实；译文不进入策略原因码、
订单身份或交易权限。输出先渲染，再保存账户与发布；部分发布失败保留已渲染文件供只读恢复。
