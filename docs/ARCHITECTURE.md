...
历史原始股份目录通过 `market/account_input.py` 验证来源、行情和缺行依据；
`market/price_series.py` 只构造因果信号，`account/corporate_actions.py` 独占权益、
分红、真实股数及红利税会计。执行、日终净值和逐日归因读取同一状态；
已有公司行动的账户拒绝券商快照覆盖。完整约束见[股份与现金输入](ACCOUNT_INPUT.md)。
