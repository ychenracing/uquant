...
普通趋势结构退出后的同股新回撤买入，还须有该次实际 SELL 之后（含成交日收盘）的
短、中期均线恢复证据。不能换一个入场理由立即绕过未恢复的结构，也不能删掉旧成交
解除这一检查；没有需要手工重置的冷却计数。MA120 与成本灾难退出不套这项短结构条件。
...
普通 CORE 新增集合须具备自身当前成熟度及有效资格证据，等分原趋势名义预算（默认 80%），
单名入场上限 50%；未成熟共享证书成员不提前占用普通入场仓位。已成交持仓及仍有效的原订单
不因成熟度标签变化而自动退出或改写；独立授权的长回撤入口仍按其自身证据判断。
