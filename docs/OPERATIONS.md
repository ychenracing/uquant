...
5. 账户高水位、资本损伤与修复 streak、风险事件、保护/恢复权重及授权证据全部保留。
   ...
   成熟快捷证书需当天强冲量或当前长期市场准入条件，并保留证券自身可信证据。
   真实行业风险发生后，成熟快捷入口须重新积累 `leader_tenure_days` 个健康市场观察；
   同日重复调用不增加确认，条件转弱则重新计数，短期冲量不能绕过尚未完成的恢复确认。
   保留 `candidate_tenure` 中的 `ordinary_market_rearm_*` 记录，不手工补写或清零。
   共享和严格独立资格仍保留各自的确认要求，该职责调整不撤销真实持仓或修复身份。
   普通修复订单实际消费授权后，`ordinary_repair_capital_active` 标记其共享资金约束。
   即使已无持仓，仍处于冻结的谨慎状态时也不能经反弹试仓例外重新占用资金；普通持仓、
   委托与责任结清且正常核心准入恢复后才解除。该记录同样须保留，迁移命令不从旧订单
   猜补修复事实。真实修复中的 `MATURE_CORE` 是独立标记的成熟资格，不转换为战略授冠。
