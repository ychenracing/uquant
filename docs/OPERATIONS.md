...
普通 CORE 的资格可来自当前已确认战略证书，或严格单名 `independent_core` 观察。
`ordinary_trend_participation` 表示当前成组趋势证书只参加普通 CORE 资金检查，不能单凭
该证书新建战略 grant；它不表示整只证券禁止买入。严格单名、同步反转、长期持续主线
以及真实修复授权仍可按各自条件申请战略部署。核对本次实际目标和订单，而不是由 grant
为空推断没有普通入场权限；已有普通持仓也不会自动排除其他合格战略候选。
...
普通新增 CORE 的目标为 20%，实际资金不足时可按原最小交易权重 5% 检查可执行增量，
最终手数、金额、费用和现金预留仍由原订单规则判断。FULL_COHORT 没有已确认主导者时，
...
战略 `CORE` 表示已有真实首次成交，仍处于核心建仓阶段；`ACTIVE` 表示已进入后续持有
阶段。FULL_COHORT 可在首笔真实成交后进入 ACTIVE；Pair/Single 的首笔订单即使完整成交，
grant 仍可能显示 `PARTIALLY_FILLED`，应同时
...
已完整完成首次入场且 epoch 仍为 CORE 的仓位，还遵守普通 CORE 的结构退出确认：
...
战略 ATR 分段信号连续为真时不逐日重复推进；相应条件在有效行情下先解除、后来
再次触发，才形成新的分段事件。缺失或非有限行情不重置触发状态。原有冲量退出、灾难
退出及独立风险减仓仍按自身条件执行。切换与重启保留已记录分段状态和真实成交证明，
不能清空状态来跳过或重放减仓。
...
5. 账户高水位、资本损伤与修复 streak、风险事件、保护/恢复权重及授权证据全部保留。
   ...
   入场按 `leader_tenure_days`（默认 5 日）确认，共享战略证书则保持其原确认要求。
