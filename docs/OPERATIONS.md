...
单名入场上限 50%；未成熟共享证书成员及未取得成熟配置的普通成熟成员另受原 20% 上限。
没有成熟配置不等于没有普通资格，仍须通过名额、冻结与真实资金检查。成熟周期已确认且无战略占用、
挂单或普通修复资本权限时，成熟集合按动态容量与原 conviction 份额分配，原高置信度条件
可使用 90%/100% 种子预算，多名各自上限 60%，仍逐项核对共同账本。已成交持仓及仍有效的原订单
不因成熟度标签变化而自动退出或改写。
...
使用当前现金恢复到本次总仓上限内。CAUTION/RECOVERY 的已确认持仓修复还要求每只实际
持仓的当前价格、快均线、5日收益和上一收盘价完整，且全部修复、无持仓损伤，并通过原
风险票数、减仓与损伤条件。恢复可能发生在历史资本回撤尚未全部收复时，不表示重新开放
普通新增风险；须区分恢复目标、待成交单和下一交易日实际成交。Sentinel
冻结仍阻止增加资本。普通待成交 BUY 若当前资格失效，其旧恢复权一并撤销；取消订单、
重启或解除冻结后都不能通过该旧权重新买入。历史恢复组合的成员形态不再单独决定
当前连续持仓的恢复权；实际恢复仍受同一行业、相关性和现金预算约束。
若原生成交记录和当前事件仍证明 recovery owner 恢复权，集中额度沿用原恢复组合上限；
这不把额度视作必须投满的目标，也不对普通恢复扩权。原逐股恢复权、单票限额、总仓、
待成交责任和实际现金均继续约束，策略退出不能伪装成风险压缩来取得补买。
...
5. 账户高水位、资本损伤与修复 streak、风险事件、保护/恢复权重及授权证据全部保留。
   ...
   历史资金修复只接受当前严格独立资格；仅有成熟观察不能申请该额度。成熟观察字段仍
   按普通成熟资格规则更新和使用，不通过补造计数授予独立修复资格。保留
   `ordinary_repair_capital_active` 和对应真实持仓、
...
风险缩仓在相同生命周期安全性与行业防守健康度下，先减少变动证券数，再比较保留效用。
这不改变风险上限、成交规则或冲击后恢复许可。
