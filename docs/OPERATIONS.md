...
长期回撤普通 CORE 另需本次 BaseRisk 明确的共享小额许可，不能把 CAUTION、现金充足或
质量 READY 自行解释成可买。`BOUNDED_PULLBACK_AUTHORIZED` 仅描述当日有限授权，仍须
本次真实 BUY；`PULLBACK_PERMISSION_CLOSED` 表示原余单的当前资格、风险或完整预算已
...
普通新增集合等分原趋势名义预算（默认 80%），单名入场上限 50%，未成熟共享证书成员
另受 20% 上限；实际增量至少达到原最小交易权重 5%。最终手数、金额、费用、现金预留及
...
5. 账户高水位、资本损伤与修复 streak、风险事件、保护/恢复权重及授权证据全部保留。
   ...
   新 `ordinary_market_session/streak` 缺失时从零观察，不从旧永久激活标记或当前持仓回填；
   它只影响普通新增资本，不撤销真实持仓或修复身份。
