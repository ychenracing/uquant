...
`--data-dir` 指向含 `LATEST` 的根目录时读取其指向的快照。

### 配股支持边界

本轮不实现配股。当前公司行动账本只覆盖现金分红、送股/转增及其红利税；
Baostock 分红接口未提供本系统所需的完整配股认购事实。配股需要认购选择、认购价格、
缴款/退款、获配数量与上市可卖日期；它不能当作免费送转，也不能仅修改 `share_ratio`。
误处理会高估现金、低估成本，并可能提前出售尚未上市的配股股份。

发现持仓涉及配股时，暂停该账户的有效决策输出，保留原账户和事件资料；先根据券商确认的
缴款、获配和可卖事实核对账本。当前没有经过验证的配股迁移命令，不能仅用普通快照或
人工买入记录声称已完整支持配股，也不能忽略 `preclose_without_event` 后继续使用该段回测。
此类提示目前是 warning，并非自动配股识别/阻断保证；使用者须检查公司行动公告。
未来需要跨越实际配股事件继续连续回放时，再以真实事件补充最小实现和资金/股数/可卖性测试。
...
`deployed_peak` 时，有正持仓则从已记录的资本高水位保守初始化，空仓则使用真实现金。
它不是新账户、资金余额或历史收益重置，不能手工下调以解除风控。完整语义见
[账户状态与决策时点](ARCHITECTURE.md#决策时点)。

## 券商快照

仅需单独对账或进行切换预演时执行以下命令；正常每日流程直接使用带 `--broker-snapshot` 的 `daily`，不要再重复同步同一份快照：
...
受当前准入预算约束的均分份额低于最小交易权重，不应先卖旧仓凑这次买入。至少一项为正且
没有仍在使用的普通修复资本时，先取 `trend_entry_gross` 与当前风险总仓上限的较小值，
再向合格新成员分配；仍在使用的修复资本继续遵守原共享预算，不能借此重新领取额度。
...
普通 CORE 新增集合按原有效资格及单一未成熟槽位规则筛选，再等分上述准入规则允许的预算；
趋势名义预算默认 80%，须先受当前风险总仓上限约束，不能把 80% 当作获准投入的固定额度。
...
4. 收盘后导出完整成交和持仓快照；
5. 下一次带 `--broker-snapshot` 的 `daily` 统一对账并生成新日报；只需单独对账时使用 `account-sync`；
6. 核对账本与日报没有未解释差异，保存对应快照与备份。
...
默认执行时钟为 `AUCTION`：数量和限价在开盘前确定。数量按前收盘 ×（1 ± 滑点）的预估成交价
折算目标权重，单票上限也按这一价格衡量；买单必须在预设限价（前收盘上浮
`auction_limit_buffer`，不超过涨停价）和最不利费用下可支付，同次竞价的卖出回款不能提前用于
买入；被资金预算削减的部分不是遗留挂单。`DAILY_PROXY` 保留旧的开盘价定量代理，
只用于标明口径的研究对比。
...
副本已经同步这份快照，随后执行上文 `daily` 时省略 `--broker-snapshot`，避免重复同步。
账户改为 `cutover_review/account.candidate.json`，输出改为
...
[三个双格式示例](examples/daily/README.md)展示买入意图、新增冻结、部分成交后卖出受阻。
这些是测试决策对象，不是实时推荐。正常日扫始终使用同一次决策生成两个格式。

### 本轮工程收敛后的操作核对

观察入口的日期校验在不可变追加之前完成；输入载体、生产账户、备份、冻结行情及 Holdout
证据不能互相覆盖。同仓库不同账户也共享一把观察事务锁。备份不会代替日常单一写入者规则，
不要同时运行 `daily`、`account-sync`、单步 holdout 或手工修改同一组文件。

真实建议与执行记录按[Journal 对应规则](HOLDOUT.md#建议执行偏差与后续结果的对应)保存。
无订单日保留日报；部分成交延续原订单身份；人工跳过不等于券商取消。报告发布失败且账户
已保存时，恢复已渲染的 ready 文件，不为补报告重跑决策。正式 Future Holdout 缺少当前身份的
前瞻登记、连续账户或真实数据时仍未完成验收，不能把日常记录数量当成正式观察数。
