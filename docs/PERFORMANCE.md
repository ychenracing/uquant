...
冻结 5 标的 champion 参考财富为 `24.509661802900865`，候选至少
保留其 95%。当前 Target、Order、Fill 与 equity 路径从完整原始证据重新核算；每次真实入场的 grant/event/epoch 归因、单次成交计账和账户对账仍须一致。
...
原始状态与账目投影，并以 `runner_success AND capability_pass` 形成唯一通过结论。
...
尾部。当前 policy 逐 cell 对比冻结 champion，并检查 intrinsic floor 与固定随机组的正收益
...
`python -m research.relative_evidence` 在同一股票池、窗口、资金和费用口径下给出 REF0a、
REF0b、REF1、REF3 参照，以及冻结的 `base`、`cost_stress`、`fill_stress` 场景；参照账户使用
生产的原始价格、次日开盘、日期化费用、合法数量、涨跌停开盘阻断和前日容量代理。
这些是新增诊断，不撤销也不放宽 15× 与 23.284× 等现行目标，不以单票集中或剔除赢家设门槛。
`python -m research.mechanism_ablation` 在同一基础版本上一次只关闭一个事先登记的机制
（行业轮动、置信度定仓、市场状态因子混合），报告指标差和首次分歧成交，不做收益寻优。
`python -m research.rule_inventory` 从治理清单和源码生成每个设置的类别与生产读点；
`python -m research.trial_index` 从 `artifacts/**` 派生试验索引（路径、SHA-256、已声明的
候选与状态），不推断未记录的试验次数。

### 性能剖析基线

同条件剖析（3 只证券、2023 上半年、单进程）显示回放时间主要在：一次性构建并缓存的
风险证据时间线约 41%，战略资格快照约 22%，pandas 标签索引约 19%。优化应先在同一窗口和
证券集合上复测，再按实际热点处理；不以牺牲因果口径或证据身份换取速度。
...
- 冻结数据 `data/frozen` 是前复权股票行情且没有公司行动记录，其回放无法表达真实现金分红、
  送转和红利税；`data-update` 生成的快照以原始价格记账，公司行动进入账户事件链（配股仍不支持）；
- 次日开盘模型不包含完整排队和冲击成本；
- 参数和风险阈值可能适应已观察市场结构；
...
当前完整账户与 champion 财富下限为 15（包含本金），绝对单账户订单上限为 40。
...
本轮只修复工程与证据问题，不再在已有冻结样本上搜索收益。PR80 的成熟资本候选和
[PR81 固定普通趋势参照](https://github.com/ychenracing/uquant/pull/81)均为 NO_GO，失败实现
不进入生产。PR80 中独立恢复证据修复已经进入 main，不与失败经济候选混淆。

已有完整 Absolute 报告的 34 个单证券剔除单元均为正收益，p10 终值为 26.0244 倍、
p90 最大回撤为 29.7827%；原报告同时存在本轮查明的配置摘要标注问题，状态与原件见
[当前验收链](ACCEPTANCE.md#已有完整结果与本轮身份修复)。这些单证券剔除结果不等于
任意弱股票池、去掉三只主导证券、不同起始日或未来市场都能通过。

此前四路径基线的成本后终值分别为全程 32.6626 倍、2024 起始 12.6305 倍、剔除
sz300308 为 4.3815 倍、去三只为 3.4101 倍。它们与 Absolute 的 champion/34 单元具有
各自合同和账户路径，不拼接比较。结果显示对起始日及主导证券的敏感性；此前未被有效接受的
候选收益、未来观察、真实公司行动与执行摩擦仍不能靠工程检查补足。

当前观察报告为零个真实 session、正式分数 `null`。日常建议、人工执行偏差和后续对账可以
沿现有入口记录，但没有真实连续数据和当前身份的前瞻登记时，不计为正式 Holdout 验收。
