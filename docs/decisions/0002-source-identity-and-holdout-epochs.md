...
重写旧 epoch。`production_wheel_v1/v2` 保留原始成员与摘要；当前 `production_wheel_v3` 登记
...
成员，但历史 wheel 混合了偶然的 ZIP 权限位，不能诚实宣称容器 byte-exact；v3 通过固定成员
顺序、时间、权限并使用 `ZIP_STORED` 消除了这个环境变量。
