> 测试夹具示例，决策日 2025-01-06；不是今日交易建议。
...
- 需要处理减仓，等待下一可交易日核对
- 决策日期：2025-01-06；行情截止：2025-01-06
- 本次有评分记录的股票：sz300308；未宣称扫描全市场
- 账户同步时点：2025-01-06；下一次执行：决策日之后的下一可交易日，具体日期与可成交条件须核对
- 实际持仓比例：20.00%；目标比例：0.00%；总仓上限：100.00%（上限不是建议满仓）
- 当前持有 1 只；目标持有 0 只；买入意图 0 项；卖出意图 1 项
- 现金余额：80,000.00 元；不等于可用买入资金，未成交卖单不释放现金或名额

## 下一可交易日需要核对的事项

- 1. sz300308：准备清仓，目标 0.00%；本日新意图，原信号日期 2025-01-06；按本次组合目标处理，具体条件见下方记录
- 逐单核对券商回报、可用资金、可卖股份、停牌／涨跌停、成交量和价格；意图不是券商已接受、可立即成交或已经成交

## 逐只股票：sz300308

- 当前情况：持有 2000 股，平均成本 10.00 元
- 最终处理：准备清仓；仅为意图，尚未确认成交
- 目标仓位：0.00%
- 目标依据：按本次组合目标处理，具体条件见下方记录
- 支持关注的龙头成熟条件：条件通过；仅支持候选评估，不等于交易许可
- 普通建仓评估：已满足本项候选条件；仍需通过风险、资金和执行检查
- 龙头证据可信程度：条件通过；观测值 90.00%，要求至少 65.00%（不是上涨概率）
- 历史行情长度：条件通过；观测 160 个交易日，要求至少 121 个交易日
- 已记录连续确认次数：\{'independent\_core': 3\}；要求：3 个交易日
- 订单规划：已满足本项候选条件；仍需通过风险、资金和执行检查
- 账户订单：部分成交；原信号 2025-01-03；记录余量 2000 股
- 最近执行记录（2025-01-06）：执行当日涨跌停限制阻止成交；此为已发生的检查，不预测下一开盘可否成交
- 重新评估：仅核对上述未满足条件是否变化；单个限制解除不保证买入，仍需下一次完整决策

## 风险及其实际影响

- 新增冻结来源：本次两项检查均未触发新增冻结
- 市场基础风险：处于降低风险档
- 新增持仓权限：本项未触发新增冻结，仍需其他检查
- 独立风险观察：未记录新增冻结；不代表要求清仓
- 独立观察资料覆盖：已就绪
- 行业风险保护：未取得
- 持仓行业当日收益：未取得
- 从账户资金高点回落的比例：未取得
...
原始原因、原因码、订单身份、配置指纹和账户事实如下。

    {
      "account": {
        ...
        "anchor_weights": {},
        "broker_as_of": "2025-01-06",
        "candidate_tenure": {},
        "capital_budget_level": 0,
        "capital_budget_repair_streak": 0,
        "capital_peak": 100000.0,
        "cash": 80000.0,
        "chronic_level": 0,
        "chronic_repair_streak": 0,
        "chronic_streak": 0,
        "code_hash": "",
        "cooldown_until": "",
        "data_hash": "",
        ...
        "order_ledger": [
          {
            ...
            "last_event": "LIMIT_BLOCKED",
            "last_update_date": "2025-01-06",
            "lifecycle": "CORE",
            "mechanism": "",
            "order_id": "O000000001",
            "origin_lifecycle": "",
            "origin_subsystem": "",
            "reason": "risk-off reduction",
            "reason_code": "strategy_target",
            "reduction_policy": "FIFO",
            "remainder_release_session": "",
            "remainder_release_shares": 0,
            "remaining_shares": 2000,
            "replaced_by": "",
            "replaces_symbol": null,
            "requested_shares": 3000,
            "side": "SELL",
        ...
        "pending_orders": [
          {
            ...
            "entry_score": 0.0,
            "epoch_id": "",
            "event_id": "evt_3d3e72b95eb68d131a5e445737df25000b266bcc09dce08a00623e21415dc514",
            "exit_kind": "strategy",
            "grant_id": "",
            "industry_at_entry": "optical",
            "industry_manifest_sha256": "03f42c5066fb8e1c7b2f8e1b7dd38d508d8053f548ebb5596317ce587d7cffd0",
            "lifecycle": "CORE",
            "mechanism": "RISK_OFF",
            "order_id": "",
        ...
        "risk_anchor_candidate_streak": 0,
        ...
        "sector_recovery_streak": 0,
        "sector_shock_dates": [],
        "shock_severity": "NORMAL",
        "shock_start_date": "",
        "shock_state": "NONE",
        ...
        "strategic_cash_rearm": {
          ...
          "repair_episode_id": "",
          "risk_reference_universe_identity": "",
          "route_consistent_absolute_quality": false,
          "status": "OBSERVING",
          "tradable_universe_identity": ""
        },
        "strategic_cohort_symbols": [],
        "strategic_cohort_targets": {},
        "strategic_epoch": 0,
        "strategic_epochs": [],
        "strategic_epochs_completed": 0,
        "strategic_exit_bands": {},
        "strategic_grant": null,
        ...
        "strategic_qualification": {
          ...
          "qualification_evidence_sha256": "",
          "qualification_last_observed_session": "",
          "qualification_quorum": "",
          "qualification_ready": false,
          "qualification_route": "",
          "qualification_signature": "",
      ...
      "decision": {
        "date": "2025-01-06",
        "decision_digest": "TEST_FIXTURE_sell-blocked",
        "opportunity": "TREND",
        "pending_orders": [
          {
            "attempts": 0,
        ...
        "risk_summary": {
          ...
          "core_allocation": {
            "final_freeze_new_risk": false,
            "scope": "FINAL_DECISION",
            "symbols": {
              "sz300308": {
                "allocation_reason": "HOLD",
                "entry": {
                  "block": "READY",
                  "checks": {
                    "confidence": {
          ...
          "fixture_label": "测试夹具示例，决策日 2025-01-06；不是今日交易建议。",
