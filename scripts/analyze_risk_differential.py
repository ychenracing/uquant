...
def _calibration(cells: list[dict[str, Any]], system: str) -> dict[str, Any]:
    ...
    for cell in cells:
        ...
        for shock in shock_starts:
            ...
            prior = [positions[date] for date in warning_dates if positions[date] <= shock_pos]
            lead = shock_pos - max(prior) if prior else None
            shock_episodes.append({"date": shock, "detected": bool(prior), "lead": lead})
    ...
    return {
        ...
        "median_lead_time": median(item["lead"] for item in detected) if detected else None,
...
def _counterfactual_summary(raw: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for policy, rows in sorted(policy_rows.items()):
        aggregate[policy] = {
            ...
            "best_acute_return_delta": max(item["acute_return_delta"] for item in rows),
            "worst_acute_return_delta": min(item["acute_return_delta"] for item in rows),
...
def main() -> int:
    ...
    cells = {item["cell_id"]: item for item in matrix["cells"] if item.get("status") == "SUCCESS"}
    ...
    for candidate, kind in transfer.items():
        ...
        sample_count = len(actionable_warning) if "freeze" in candidate else len(warning_episodes)
        sample_pass = sample_count >= 5
        economic_pass = bool(
            economic
            ...
            and (economic["median_mdd_delta"] >= 0.005 or economic["best_acute_return_delta"] >= 0.01)
        )
        gate = {
        ...
        decisions.append(
            {
                ...
                "transfer_kind": kind,
                "exclusive_episode_count": len(warning_episodes),
                "actionable_exclusive_episode_count": len(actionable_warning),
                "gates": gate,
    ...
    closure = _seal(
        {
            ...
            "trade_material_incremental_capabilities": incremental,
            ...
            "final_decision": (
                ...
                else "NO_PROMOTABLE_INCREMENTAL_RISK"
            ),
        }
    )
