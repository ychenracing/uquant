...
def _prepend_tech_proxy(
    ...
    scale = target_close / Decimal(proxy_anchor[-1][4])
    ...
    prefix = "".join(
        (
            f"{date},{Decimal(open_) * scale:.6f},{Decimal(high) * scale:.6f},"
            f"{Decimal(low) * scale:.6f},{Decimal(close) * scale:.6f},"
            f"{_format_volume(TECH_PROXY, volume)},\n"
        )
        for date, open_, high, low, close, volume in sorted(proxy.values())
    )
    extended = (HEADER + prefix + text[len(HEADER) :]).encode("utf-8")
    updated = BackfillResult(
        ...
        pre_inception_proxy=f"{TECH_PROXY} close-scaled at {first_date}",
    )
...
def _write_metadata(data_dir: Path, results: list[BackfillResult]) -> None:
    ...
    manifest = {
        ...
        "adjustment": "Tencent qfq for stocks; Tencent unadjusted for indices",
        ...
        "pre_inception_index_proxy": {
            "target": TECH_INDEX,
            "proxy": TECH_PROXY,
            "rule": "proxy OHLC scaled by the common first-session close; target history used from inception",
        },
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.tech_proxy_only:
        ...
        results = [BackfillResult(**item) for item in manifest["results"]]
