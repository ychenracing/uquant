...
def _assemble(*, root: Path, runtime: Path, dry_run: bool) -> dict[str, Any]:
    external = _external_paths(runtime)
    return assemble_evidence_artifacts(
        root=root,
        output_dir=root / _ARTIFACT_DIR,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--runtime-dir", type=Path, default=Path("/tmp/uquant-strategic-evidence"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "validate":
        supplied = {
            task: path
            for task, path in _external_paths(runtime).items()
            if path.is_file()
        }
        _print(
            validate_evidence_artifacts(
    ...
    if args.dry_run:
        if phase == "assemble":
            _print(_assemble(root=root, runtime=runtime, dry_run=True))
            return 0
        _print(
            {
                "dry_run": True,
    ...
    for name in selected:
        subprocess.run(commands[name], cwd=root, check=True)  # noqa: S603
    result = _assemble(root=root, runtime=runtime, dry_run=False) if phase in {"assemble", "all"} else {
        "runner_success": True,
