...
TASK10_START_COMMIT = "a6a77deb7ae6c3bb0878895729e9a5a72cf75482"
TASK10_START_TREE = "cd3551ab769dece496b6599210a0dd14c9cd98ad"
TASK10_INVENTORY_PATH = (
...
def load_inventory(path: Path = TASK10_INVENTORY_PATH) -> dict[str, object]:
...
def build_inventory_from_immutable_git(root: Path = ROOT) -> dict[str, object]:
    archive = subprocess.run(
        ["git", "archive", "--format=tar", TASK10_START_COMMIT],
        cwd=root,
        check=True,
        capture_output=True,
    ).stdout
    with tempfile.TemporaryDirectory(prefix="uquant-task10-inventory-") as raw:
        ...
        return build_inventory(
            extracted,
            start_commit=TASK10_START_COMMIT,
            start_tree=TASK10_START_TREE,
        )
...
def _main() -> None:
    ...
    TASK10_INVENTORY_PATH.parent.mkdir(parents=True, exist_ok=True)
    TASK10_INVENTORY_PATH.write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(f"{TASK10_INVENTORY_PATH}: {payload['artifact_sha256']}")
