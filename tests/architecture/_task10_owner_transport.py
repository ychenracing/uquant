...
_TASK8_REVIEW_COMMIT = "c0cde6c60bbf234d08e836f84981aa1b3231279b"
...
_SENTINEL_ADDITIONS = frozenset(
    {
        "uquant/account/validation_attribution.py",
        "uquant/risk_sentinel/history_cache.py",
        "uquant/risk_sentinel/legacy_surface.py",
    }
)
...
TASK10_SOURCE_SURFACE_ADDITIONS: Mapping[str, frozenset[str]] = {
...
def task10_source_surface_projection(identifier: str, historical: Set[str]) -> set[str]:
    """Add only the exact Task-10 owners registered for one frozen surface."""
    assert identifier in TASK10_SOURCE_SURFACE_ADDITIONS
    additions = TASK10_SOURCE_SURFACE_ADDITIONS[identifier]
...
def task10_task7_reviewed_sources(
    *,
    root: Path,
    ...
    reviewed = {
        relative: source
        for relative, source in git_python_sources(root, _TASK7_REVIEW_COMMIT).items()
...
def task10_task8_reviewed_sources(
    *,
    root: Path,
    overrides: Mapping[str, str] | None = None,
) -> dict[str, str]:
    ...
    reviewed = {
        relative: source
        for relative, source in git_python_sources(root, _TASK8_REVIEW_COMMIT).items()
...
def task10_private_relocation_projection(
    *,
    root: Path,
    task: int,
    observed: Set[str],
    ...
    moves = _TASK7_PRIVATE_MOVES if task == 7 else _TASK8_PRIVATE_MOVES
    assert task in {7, 8}
    if task == 7:
        overrides = task10_task7_reviewed_sources(root=root, overrides=overrides)
    else:
        overrides = task10_task8_reviewed_sources(root=root, overrides=overrides)
    missing = set(expected) - set(observed)
    expected_missing = set(moves)
    if task == 7:
        expected_missing.add(_TASK7_RENAMED_MOVE[0])
...
def _task10_start_pipeline(root: Path) -> ast.FunctionDef:
...
def expand_task10_portfolio_pipeline(
    *,
    root: Path,
    ...
    overrides = task10_task8_reviewed_sources(root=root, overrides=overrides)
    frozen = _task10_start_pipeline(root)
...
def expand_task10_risk_market_stage(
    *,
    root: Path,
    ...
    frozen_source = subprocess.check_output(
        ["git", "show", f"{TASK10_START_COMMIT}:uquant/risk/assessment.py"],
        cwd=root,
        text=True,
    )
...
def expand_task10_risk_stage(
    *,
    ...
    if stage_name == "_assess_market_and_book_evidence":
        return expand_task10_risk_market_stage(
            root=root,
            wrapper=wrapper,
            overrides=overrides,
        )
...
def expand_task10_risk_assessment(
    *,
    root: Path,
