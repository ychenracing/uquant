...
from ._task10_owner_transport import task10_task8_reviewed_sources
...
def expand_task10_checkpoint1_method(
    *,
    root: Path,
    relative: str,
    name: str,
...
def task10_task8_type_ignore_projection(
    *,
    root: Path,
    observed: Set[str],
    expected: Set[str],
    ...
    assert observed == set()
    frozen_source = subprocess.check_output(
        ["git", "show", f"{TASK10_START_COMMIT}:uquant/portfolio/risk_reduction.py"],
        cwd=root,
        text=True,
    )
