...
from ._task10_private_imports import scan_sealed_governed_private_edges

_IMMUTABLE_GLOBALS = "ec8d4b7d1502ad50a73deab4543480b8b01f7d03"
_ATTRIBUTION_SPLIT = "32af12b93b8981343a97a1754b67da8331c123ba"
_TASK5_PUBLIC_OWNERS_COMMIT = "fd8863d50635e1bd5a80eba8ba9b14fc6a6b17bb"
_TASK5_PUBLIC_OWNERS_TREE = "41bced003106d67e5061713b6f25e9fbb14d64d6"
...
def task5_reviewed_source(root: Path, relative: str) -> str:
...
def task5_post_checkpoint_private_edges(
    root: Path,
    ...
    return scan_sealed_governed_private_edges(
        root,
        commit=_TASK5_PUBLIC_OWNERS_COMMIT,
        tree=_TASK5_PUBLIC_OWNERS_TREE,
    )
    ...
    for relative, chain in _REVIEWED_PATH_CHAINS.items():
        log = subprocess.check_output(
            [
                ...
                "log",
                "--format=%H",
                f"{TASK10_START_COMMIT}..{_ATTRIBUTION_SPLIT}",
                "--",
                relative,
            ],
            cwd=root,
        ...
        reviewed = task5_reviewed_source(root, relative)
        candidate = (
            overrides[relative]
...
def _assert_validation_public_transport(
    ...
    immutable = _tree(
        _git_source(root, TASK10_START_COMMIT, "uquant/attribution/validation.py")
    )
...
def _assert_empty_bucket_absorption(
    ...
    immutable_builder = _function(
        _tree(_git_source(root, TASK10_START_COMMIT, "uquant/attribution/builder.py")),
    ...
    immutable_concentration = _tree(
        _git_source(root, TASK10_START_COMMIT, "uquant/attribution/concentration.py")
    )
...
def validate_task5_private_transport(
    *,
    root: Path,
...
def task5_private_relocation_projection(
    *,
    root: Path,
    expected: Set[str],
    observed: Set[str],
    ...
    validate_task5_private_transport(root=root, source_overrides=source_overrides)
