...
from ._task10_task9_capability_projection import (
    CAPABILITY_OWNERS,
...
def reviewed_task9_owner_source(
    root: Path,
    *,
    owner: str,
    ...
    immutable = _git_source(root, TASK10_START_COMMIT, owner)
    previous = immutable
...
def assert_reviewed_task9_owner_transport(root: Path) -> None:
    for owner in _REVIEWED_CHAINS:
        reviewed_task9_owner_source(root, owner=owner)
...
def assert_task9_importer_public_transports(
    *,
    root: Path,
...
def task9_private_relocation_projection(
    *,
    root: Path,
    observed: Set[str],
    expected: Set[str],
...
def task9_historical_debt_projection(
    *,
    root: Path,
    ...
    assert_reviewed_task9_owner_transport(root)
