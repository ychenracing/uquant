...
_CONFIG_REVIEW_COMMIT = "5cca5a3f1cc1866d15c86d674098f800c1420995"
    ...
    *,
    ...
    reviewed_source = subprocess.check_output(
        [
            "git",
            "show",
            f"{_CONFIG_REVIEW_COMMIT}:tests/architecture/_analysis_debt.py",
        ],
        cwd=root,
        text=True,
    )
...
def _risk_legacy_unit(
    *,
    ...
    if name == "_assert_risk_ownership_surface":
        rewrites = (
            (
...
def risk_legacy_unit_counts(
    *,
    ...
    units = (
        ("_assert_risk_ownership_surface", relative),
        (
...
def verify_test_relocations(
    *,
    ...
    for source, targets in relocation_paths.items():
        ...
        elif source == "tests/architecture/test_risk_boundaries.py":
            observed = risk_legacy_unit_counts(
                immutable_source=immutable_risk_source,
                current_sources=current_sources,
            )
        else:
            observed = Counter(
