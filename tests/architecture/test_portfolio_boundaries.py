...
''').body
    offset = next(index for index, item in enumerate(projected.body)
                  if isinstance(item, ast.Assign) and any(
                      isinstance(target, ast.Name) and target.id == "holding_return" for target in item.targets))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("holding_return < min(0.0, reference_return)", "holding_return < max(0.0, reference_return)"),
