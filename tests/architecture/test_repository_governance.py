...
def test_current_engineering_paths_use_domain_responsibilities() -> None:
    staged_paths = sorted(
        relative
        for relative in _tracked_contents()
        if relative.startswith(("research/", "scripts/", "tests/"))
        and _PROJECT_STAGE_PATH.search(relative.rsplit("/", 1)[-1])
    )

    assert staged_paths == []
    assert not (ROOT / "uquant/risk_sentinel/legacy_surface.py").exists()
    assert (ROOT / "uquant/risk_sentinel/source_identity_archive.py").is_file()
    for package in ("contracts", "validation"):
        resources = ROOT / "uquant" / package / "resources"
        assert not (resources / "phase1_frozen_champion.json").exists()
...
def test_repository_distribution_declares_only_the_production_namespace() -> None:
...
def test_repository_console_pytest_collects_non_distribution_research_tests() -> None:
    completed = subprocess.run(
        [
            "uv",
            "run",
...
def test_repository_repository_only_cli_examples_use_module_execution() -> None:
    governed_examples = (
...
def test_repository_canonical_docs_have_resolved_internal_links_and_current_authority_terms() -> None:
...
def test_repository_inventory_paths_are_tracked_and_reference_evidence_is_reproducible() -> None:
