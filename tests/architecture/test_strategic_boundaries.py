...
from ._owner_transport import architecture_private_relocation_projection
...
def test_portfolio_strategic_strategic_owners_and_thin_facade_are_complete() -> None:
    assert all((ROOT / path).is_file() for path in _STRATEGIC_PACKAGE_PATHS)
...
def test_portfolio_strategic_strategic_methods_and_target_slices_are_ast_exact() -> None:
    ...
    assert set(delegations) == _STRATEGIC_TARGET_HELPERS
    expected_keywords = {
    ...
    assert set(targets) == _STRATEGIC_TARGET_HELPERS
...
def test_portfolio_strategic_ast_gate_rejects_strategic_rule_mutations() -> None:
    immutable = _immutable_methods()
...
def test_portfolio_strategic_private_and_complexity_relocations_are_closed() -> None:
    expected = {
        ...
        for path, names in _STRATEGIC_OWNER_METHODS.items()
        for name in names
    }
    observed = {
        identifier: legacy
    ...
    assert set(observed.values()) == {
        ...
        for names in _STRATEGIC_OWNER_METHODS.values()
        for name in names
    }
