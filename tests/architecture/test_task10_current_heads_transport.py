...
from ._task10_cli_transport import current_heads_adapter_transport_unit_digests
...
def _frozen_source() -> str:
    return subprocess.check_output(
        ["git", "show", f"{TASK10_START_COMMIT}:{_FROZEN_RELATIVE}"],
        cwd=ROOT,
        text=True,
    )
...
def test_task10_current_heads_transport_rejects_unknown_public_adapter_mutation(
    relative: str,
    original: str,
    mutation: str,
) -> None:
