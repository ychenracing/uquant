...
def _immutable_public_script_definitions(relative: str) -> tuple[str, ...]:
    source = subprocess.check_output(
        ["git", "show", f"{TASK10_START_COMMIT}:{relative}"],
        cwd=ROOT,
        text=True,
    )
    return tuple(
...
def test_task10_private_import_inventory_accounts_for_all_relocation_buckets() -> None:
    payload = load_inventory()
...
def test_task10_raw_scanner_treats_a_relocated_id_as_current_live_debt() -> None:
    known_relocated_id = (
        "uquant.account.codec:uquant.account.validation_common:_finite_number"
    )
    assert known_relocated_id in _TASK5_RELOCATED_PRIVATE_IMPORTS
    mutation = {
...
def test_task10_live_analyzer_separates_current_debt_from_historical_projection() -> None:
    known_relocated_id = (
    ...
    for task, frozen_ids in (
        (5, _TASK5_RELOCATED_PRIVATE_IMPORTS),
        (6, _TASK6_RELOCATED_PRIVATE_IMPORTS),
        (7, _TASK7_RELOCATED_PRIVATE_IMPORTS),
        (8, _TASK8_RELOCATED_PRIVATE_IMPORTS),
        (9, _TASK9_RELOCATED_PRIVATE_IMPORTS),
    ):
...
def test_task10_live_analyzer_measures_nonproduction_direct_private_edges(
    probe_path: str,
) -> None:
...
def test_task10_live_analyzer_measures_qualified_private_edges(
    governed_root: str,
) -> None:
    sources = current_governed_sources()
...
def test_task10_raw_scanner_rejects_qualified_private_import_evasion(
    source: str,
) -> None:
    mutation = {
...
def test_task10_raw_scanner_rejects_reviewed_fail_closed_bypasses(
    source: str,
    owner: str | None,
    bucket: str,
    evidence: str,
...
def test_task10_raw_scanner_rejects_dynamic_private_transport_evasion(
    governed_root: str,
    source: str,
    kind: str,
) -> None:
...
def test_task10_raw_scanner_accepts_only_literal_bounded_star_exports() -> None:
    mutation = {
        "uquant/account/codec.py": (
            "from uquant.account.validation_common import *\n"
...
def test_task10_raw_scanner_accepts_bounded_local_export_reflection() -> None:
    mutation = {
        "uquant/account/codec.py": (
            "from uquant.account.validation_common import *\n"
            "__all__ = ('finite_number',)\n"
...
def test_task10_raw_scanner_rejects_unproved_star_exports(
    declaration: str,
) -> None:
    mutation = {
        "uquant/account/codec.py": (
...
def test_task10_governed_scripts_expose_only_frozen_public_start_surface() -> None:
    assert {
...
def test_task10_governed_scripts_have_no_dynamic_private_transport() -> None:
...
def test_task10_risk_facade_exposes_two_explicit_runtime_capabilities(
...
def test_task10_generalization_facades_expose_finite_runtime_capabilities(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_task10_holdout_facades_expose_only_finite_typed_capabilities(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_task10_final_raw_private_debt_matches_canonical_acceptance_allowlist() -> None:
