...
from ._task10_private_imports import current_governed_sources, scan_governed_private_edges
from ._task10_task5_transport import task5_post_checkpoint_private_edges
...
def test_task10_task5_public_routes_preserve_exact_owner_identity() -> None:
    ...
    assert len(rows) == 123
    assert original_pairs == set(TASK5_PUBLIC_ROUTES)
    for (legacy_owner, private_name), (public_owner, public_name) in sorted(
        TASK5_PUBLIC_ROUTES.items()
    ):
...
def test_task10_task5_importers_keep_exact_local_legacy_bindings() -> None:
    graph = architecture_snapshot()["import_graph"]
    assert isinstance(graph, Mapping)
    rows = graph["task5_relocated_private_imports"]
    assert isinstance(rows, list)
    for row in rows:
        ...
        public_owner, public_name = TASK5_PUBLIC_ROUTES[(legacy_owner, private_name)]
...
def test_task10_task5_raw_private_edges_are_closed() -> None:
    checkpoint = task5_post_checkpoint_private_edges(ROOT)
