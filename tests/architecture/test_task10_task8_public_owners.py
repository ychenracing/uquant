...
_TASK8_PUBLIC_OWNERS_COMMIT = "a2a81c2729d0f487e4816006beb4967dfd169cb2"
_TASK8_PUBLIC_OWNERS_TREE = "08eb399120d76626cca3c9618cbb76b564d6a652"
...
def _task8_rows() -> list[Mapping[str, object]]:
...
def test_task10_task8_public_routes_preserve_exact_owner_identity() -> None:
    rows = _task8_rows()
    pairs = {(str(row["imported_from"]), str(row["name"])) for row in rows}
    assert len(rows) == 35
    assert pairs == set(TASK8_PUBLIC_ROUTES)
    for (legacy_owner, private_name), (public_owner, public_name) in sorted(
        TASK8_PUBLIC_ROUTES.items()
    ):
...
def test_task10_task8_importers_keep_exact_local_legacy_bindings() -> None:
    for row in _task8_rows():
        legacy_owner = str(row["imported_from"])
        private_name = str(row["name"])
        public_owner, public_name = TASK8_PUBLIC_ROUTES[(legacy_owner, private_name)]
...
def test_task10_task8_current_private_edges_are_closed() -> None:
    historical_ids = {str(row["id"]) for row in _task8_rows()}
...
def test_task10_task8_raw_count_progression_is_exact() -> None:
    checkpoint = scan_sealed_governed_private_edges(
        ROOT,
        commit=_TASK8_PUBLIC_OWNERS_COMMIT,
        tree=_TASK8_PUBLIC_OWNERS_TREE,
    )
