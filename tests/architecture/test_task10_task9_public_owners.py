...
from ._task10_task9_transport import assert_task9_importer_public_transports
...
def test_task10_task9_inventory_and_public_owner_routes_are_exact() -> None:
    graph = architecture_snapshot()["import_graph"]
    assert isinstance(graph, Mapping)
    task9 = graph["task9_relocated_private_imports"]
    assert isinstance(task9, list) and len(task9) == 178
    direct = _rows("direct")
...
def test_task10_task9_direct_importers_keep_exact_local_legacy_bindings() -> None:
    rows = _rows("direct")
    assert_task9_importer_public_transports(
        root=ROOT,
        rows=rows,
...
def test_task10_task9_importer_transport_rejects_unknown_binding(
    relative: str,
    original: str,
    mutation: str,
) -> None:
...
def test_task10_task9_closes_all_raw_private_edges() -> None:
