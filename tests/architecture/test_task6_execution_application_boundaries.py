...
def _immutable_python_sources() -> dict[str, bytes]:
    paths = [
        ...
        for path in subprocess.run(
            ["git", "ls-tree", "-r", "--name-only", _TASK6_START],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
    ...
    batch = subprocess.run(
        ["git", "cat-file", "--batch"],
        cwd=ROOT,
        input="".join(f"{_TASK6_START}:{path}\n" for path in paths).encode(),
        check=True,
        capture_output=True,
    ).stdout
...
def test_task6_inventory_is_bound_to_immutable_blobs_and_reference_sets() -> None:
    ...
    immutable_registry = json.loads(
        subprocess.run(
            [
                "git",
                "show",
                f"{_TASK6_START}:benchmarks/source_surface_registry.json",
            ],
            cwd=ROOT,
            check=True,
            capture_output=True,
    ...
    assert payload["baseline_commit"] == _TASK6_START
    ...
    for entry in payload["entries"]:
        ...
        references = subprocess.run(
            ["git", "grep", "-l", "--fixed-strings", entry["path"], _TASK6_START, "--", "."],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
...
def test_task6_source_surface_migration_is_exact_for_all_five_v1_surfaces() -> None:
...
def test_task6_responsibility_owners_replace_execution_monolith_and_engine_bodies() -> None:
...
def test_task6_facade_and_decision_fanout_are_bounded() -> None:
...
def test_task6_private_edges_are_exactly_bound_to_the_mechanical_split() -> None:
    ...
    assert {str(row["id"]) for row in relocated} == _TASK6_RELOCATED_PRIVATE_IMPORTS

    task6_prefixes = ("uquant.application", "uquant.engine", "uquant.execution")
    baseline = json.loads(
    ...
    assert (
        not {
            str(row["id"])
            for row in ordinary
            if str(row["importer"]).startswith(task6_prefixes)
            or str(row["imported_from"]).startswith(task6_prefixes)
        }
        - allowed
    )
    assert not {
...
def test_task6_complexity_debt_relocations_are_exact_and_do_not_create_an_exemption() -> None:
    snapshot = architecture_snapshot()
    ...
    assert {legacy for legacy, _overhead in _TASK6_RELOCATED_FUNCTION_DEBT.values()} == immutable
    assert {
        current: overhead
        for current, (_legacy, overhead) in _TASK6_RELOCATED_FUNCTION_DEBT.items()
        if overhead
...
def test_task6_moved_definitions_are_mechanically_bound_to_immutable_source() -> None:
...
def test_task6_public_reflection_and_pickle_identities_remain_legacy() -> None:
