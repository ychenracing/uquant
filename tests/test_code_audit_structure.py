...
def test_engine_decision_uses_current_dependency_seams(tmp_path, monkeypatch):
    ...
    assert (
        instance.decide(symbols=("sz300308",), as_of="2026-01-06", account=AccountState.empty(1000.0))
        is sentinel
    )
...
def test_typed_opportunity_flow_matches_base_outputs_and_mutations(data_dir):
    ...
    panel = {
        symbol: compute_features(store.load(symbol), cfg) for symbol in ("sz300308", "sz300502", "sz300394")
    }
    leaders = {
        symbol: LeaderScore(symbol, 0.95 - i * 0.08, 1.0, True, False, "optical", {})
