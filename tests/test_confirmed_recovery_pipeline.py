...
def test_actual_tactical_probe_precedes_confirmed_recovery_and_fills(recovery_prefix):
    ...
    # The shared queue splits the recovery gross left beside the held probe by strength.
    assert fresh == pytest.approx({"sz300394": .08414895035321496, "sz300502": .2358510496467851})
...
def test_recovery_admission_cannot_bypass_protected_owner_structure(recovery_prefix):
    ...
    assert all(row["restore_block"] == "STRUCTURE_NOT_REPAIRED"
               for symbol, row in risk.evidence["core_allocation"]["symbols"].items()
               if symbol in account.protected_weights)
