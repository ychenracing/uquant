...
def _fund_recovery(book, targets):
    _admit_new_cores(book, candidates=[], opportunity=Opportunity.WEAK, market={}, recovery=targets)
...
@pytest.mark.parametrize("held,cash,expected", [(.65, .35, (.16, .11)), (.6, .4, (.16, .16)), (.88, .12, (0., 0.)), (.6, .02, (0., 0.))])
def test_fresh_peer_budget_uses_score_code_order_and_keeps_minimum(held, cash, expected):
    book, names, _ = _book(held, cash)
    ...
    for ordered in (targets, tuple(reversed(targets))):
        ...
        _fund_recovery(current, ordered)
        ...
        for name, weight in zip(names[1:], expected, strict=True):
            assert current.proposed.get(name, 0.) == pytest.approx(weight)
...
def test_subminimum_request_leaves_real_cash_for_next_eligible_peer():
    book, names, _ = _book(.65, .27)
    ...
    assert book.proposed[names[2]] == pytest.approx(.27)
