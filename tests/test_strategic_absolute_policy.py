...
def _forced_owner_summary(status: str = "SUCCESS") -> dict[str, Any]:
    controls = [
        {"control_id": f"POSITIVE_CONTROL:{owner}", "owner": owner}
        for owner in CONTRACT.positive_controls
    ]
    controls.extend(
        {"control_id": control_id, "owner": owner}
    ...
    for control in controls:
        prefix = control["control_id"]
        if prefix.startswith("POSITIVE_CONTROL:"):
            prefix = f"{prefix}:{control['owner']}"
        for mode in ("COMMON_ACTIVATION_DATE", "NATIVE_ELIGIBILITY_DATE"):
            cells.append(
                {
                    "cell_id": f"{prefix}:{mode}",
                    "status": status,
                    "metrics": {
                        "final_wealth": 2.0,
                        "max_drawdown": 0.1,
            ...
            rows.append(
                {
                    ...
                    "analysis": {
                        ...
                        "findings": [
                            {"observation_id": f"R{index}", "observed": True}
...
def test_reachability_does_not_turn_84_successes_into_capability_pass() -> None:
    ...
    for row in rows:
        finding = next(
            finding
            for finding in row["analysis"]["findings"]
            if finding["observation_id"] == "R7"
        )
