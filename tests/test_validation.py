...
def test_authorized_order_revision_is_bounded_and_preserves_risk() -> None:
    gate = promotion_module.AI_ERA_POLICY["official"]["continuous_ai_era"]
    ...
    assert basis["maximum"] == 40
    assert basis["scenario"] == "all_account_replays"
    assert basis["authorization_id"] == "cross-ai-principal15-orders40-20260910-v4"
    assert promotion_module._hard_violations(
        name="e/continuous_ai_era", metrics=metrics, gate=gate, authorized=False,
    )
