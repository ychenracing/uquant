...
def _fresh_risk_budget(book: AllocationBook, requests: dict[str, float], budget: float) -> dict[str, float]:
    """Scale fresh demand by current percentage ATR, preserving request caps."""
    ...
    for symbol, demand in requests.items():
        ...
        if frame is None or book.date not in frame.index:
            return fallback
        close, atr = (scalar(frame.loc[book.date], key, math.nan) for key in ("close", "atr"))
        if not all(math.isfinite(value) and value > 0 for value in (close, atr)):
            return fallback
        scores[symbol] = demand * close / atr
...
def _fund_members(book: AllocationBook, targets: tuple[Target, ...], members: set[str],
    ...
    if len(requests) > 1 and len(industries) == 1:
        ...
        increments = _fresh_risk_budget(book, requests, remaining)
